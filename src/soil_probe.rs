...
use crate::pins::{
    SOIL_EXCITATION_DUTY_PCT, SOIL_EXCITATION_SETTLE, SOIL_SAMPLE_COUNT, SOIL_SAMPLE_GAP,
};
    ...
    PIN: AdcChannel,
{
    ...
    /// `SOIL_SAMPLE_COUNT` calibrated reads, excitation off. Takes roughly
    /// settle + `SOIL_SAMPLE_COUNT * SOIL_SAMPLE_GAP`, but - unlike a
    /// blocking ADC read - yields to the executor the whole time instead of
    /// busy-spinning, so it never starves the Thread/Matter stack sharing
    /// this single-core executor.
